...
| CMD | Name | Args | Description |
|-----|------|------|-------------|
| 0x00 | SLOT_POKE | 5: A0=slot, A1/A2/A3=24-bit address (little-endian), A4=byte | Writes a single byte into the specified RAM slot at the specified address. May be used for patching vectors or other known locations prior to activating that slot or entering command-response mode. |
...
3. Issues SLOT_POKE commands to patch NMI/IRQ/reset vectors in RAM slot 1.
4. Issues CONFIG_CMD_RESP with format=0x01 and chosen complete/status-OK bytes.
5. Issues ENTER_CMD_RESP.
6. Polls token LSB then progress to confirm command-response mode is active.
7. Issues GET_FLASH_SLOT_INFO.
8. Reads flash slot count, names and types from the response region.
9. **Auto-boot path:** selects target slot, issues LOAD_SLOT then SWITCH_AND_EXIT.
10. **Menu path:** presents a menu, scanning the keyboard as needed (exiting and re-entering command-response mode via a full knock each time to service IRQs), then issues LOAD_SLOT and SWITCH_AND_EXIT on selection.
11. The device activates the new ROM slot. The bootloader jumps through the reset vector of the newly loaded ROM.
